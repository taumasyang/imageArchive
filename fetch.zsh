...
do
	...
	print -- "$enddate,$filename,$mkt,$(grep -oE '[0-9]+' <<< ${urlBase##*_}),\"$(xmllint --xpath '//headline/text()' - <<< $image | head -n$n)\",\"$(xmllint --xpath '//copyright/text()' - <<< $image | head -n$n)\"" >> metadata/$mkt.csv
